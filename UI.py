        ...
        theme: {
          extend: {
            ...
            "fontSize": {
                    ...
                    "headline-lg": ["20px", {"lineHeight": "26px", "fontWeight": "700"}],
                    "body-lg": ["13px", {"lineHeight": "18px", "fontWeight": "400"}],
                    "body-md": ["11px", {"lineHeight": "16px", "fontWeight": "400"}],
...
<style>
        ...
        .spinner-btns button {
            width: 16px; height: 13px; display: flex; align-items: center; justify-content: center;
            background: rgba(225, 232, 253, 0.9); border: 1px solid #c3c6d7; border-radius: 3px;
            cursor: pointer; color: #434655; font-size: 7px; line-height: 1; padding: 0; transition: background 0.15s;
        }
...
<header class="flex justify-between items-center px-6 h-11 w-full bg-surface-container-lowest border-b border-outline-variant shrink-0">
    <div class="flex items-center gap-2">
        <span class="font-headline-sm text-headline-sm font-bold text-on-surface">8-Puzzle Solver Simulator</span>
    </div>
    <nav class="hidden md:flex items-center gap-4">
        <button class="flex items-center gap-1 text-primary font-bold border-b-2 border-primary pb-0.5 font-label-md text-label-md">
            <span>BFS (Uninformed)</span>
        </button>
    </nav>
</header>

<main class="flex-1 w-full p-4 flex items-center justify-center overflow-y-auto custom-scrollbar">
<div class="grid grid-cols-1 lg:grid-cols-12 gap-4 w-full max-w-[1100px] mx-auto items-start">
    <div class="lg:col-span-7 flex flex-col gap-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <section class="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 shadow-sm">
                ...
                <div class="grid grid-cols-3 gap-2 mb-3" id="initial-grid"></div>
            ...
            <section class="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 shadow-sm">
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">1</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">2</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">3</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">4</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">5</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">6</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">7</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-low rounded-lg font-headline-sm text-headline-sm text-on-surface border border-outline-variant/20">8</div>
                    <div class="h-9 flex items-center justify-center bg-surface-container-lowest rounded-lg font-label-sm text-label-sm text-outline-variant border border-dashed border-outline-variant"><br></div>
                </div>
            </section>
        </div>
        ...
        <section class="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 shadow-sm flex flex-col items-center">
            <div class="flex items-center justify-between border-b border-outline-variant pb-2 mb-2 w-full">
                ...
                <span id="step-label" class="px-2 py-0.5 bg-surface-container-high text-on-surface-variant rounded font-label-sm text-label-sm">Ready</span>
            </div>
    ...
    <div class="lg:col-span-5 flex flex-col gap-4">
        ...
        <section class="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 shadow-sm">
            <div class="flex items-center justify-between mb-2 border-b border-outline-variant pb-2">
        ...
        <section class="bg-surface-container-high border border-outline-variant rounded-xl flex flex-col shadow-sm h-[200px]">
            ...
            <div id="execution-log" class="flex-1 p-3 font-mono text-[10px] leading-relaxed overflow-y-auto custom-scrollbar text-[#0f172a]">
...
function buildInitialGrid(values) {
    ...
    for(let i = 0; i < 9; i++) {
        ...
        input.type = 'number';
        input.id = `cell-${i}`;
        input.className = 'h-9 w-full text-center bg-surface-container-high rounded-lg font-headline-sm text-headline-sm text-primary border border-primary/20 focus:outline-none focus:ring-2 focus:ring-primary';
...
function formatLogState(title, state, action=null) {
    ...
    let titleHtml = action ? `<p class="mb-1 font-semibold" style="font-size:11px;">Bước ${title}: Di chuyển ô trống sang <span class="text-primary">${action}</span></p>` 
                           : `<p class="mb-1 font-semibold" style="font-size:11px;">${title}</p>`;
                           
    return `
    <div>
        ${titleHtml}
        <div class="bg-surface-container-low p-1.5 rounded border border-outline-variant/30 inline-block">
            <pre class="leading-tight text-[10px]">${stateStr}</pre>
        </div>
    </div>
...
async function animatePath(start_state, path) {
    ...
    document.getElementById('step-label').innerHTML = `<span style="background:#d1fae5; color:#065f46; padding:2px 6px; border-radius:4px; font-size:10px; font-weight:600;">Finished</span>`;
}
...
async function callBfs(mode) {
    ...
    if (result.success) {
        ...
        let logHtml = `<div class="text-primary font-bold mb-2" style="font-size:11px;">ĐANG GIẢI BẰNG: ${mode.toUpperCase()} GOAL TEST</div>`;
        ...
        document.getElementById('execution-log').innerHTML = '<div class="text-red-600 font-bold" style="font-size:11px;">Không tìm thấy giải pháp!</div>';
        document.getElementById('stat-steps').innerText = "-";
        document.getElementById('stat-nodes').innerText = result.nodes.toLocaleString();
        document.getElementById('stat-time').innerText = result.time + "ms";
        document.getElementById('stat-depth').innerText = "-";
        renderBoard(start_state);
        document.getElementById('step-label').innerHTML = `<span style="background:#fee2e2; color:#991b1b; padding:2px 6px; border-radius:4px; font-size:10px; font-weight:600;">No Solution</span>`;
    }
}
...
if __name__ == '__main__':
    ...
    window = webview.create_window(
        ...
        html=html_content, 
        js_api=api,
        width=1040, # Đã bóp nhỏ chiều ngang 
        height=620, # Đã hạ chiều cao xuống vừa khít
        resizable=True
    )
    webview.start()